...
Only rows with Import = Yes are emitted. The script edits one bracketed,
auto-generated block in mods/pokered-lzh/main.lua and leaves hand-written
localisation code alone.
...
def selected_rows(workbook: Path):
    ...
    for sheet_row, row in enumerate(rows, start=2):
        ...
        if not label:
            errors.append(f"{where}: Import=Yes but Text Label is blank")
            continue
        if not translation.strip():
            errors.append(f"{where} {label}: Import=Yes but Translation is blank")
            continue
        if label in seen:
            errors.append(f"{where} {label}: duplicate; first imported at row {seen[label]}")
            continue
        seen[label] = sheet_row
        ...
        selected.append((label, translation, area, status, sheet_row))
    return sheet_name, selected, errors, len(rows)


def build_block(rows, workbook_name: str) -> str:
    out = [
        BEGIN,
    ...
    for label, translation, area, status, _ in rows:
...
def main() -> int:
    ...
    try:
        sheet_name, rows, errors, total = selected_rows(args.workbook)
    ...
    try:
        original = args.main_lua.read_text(encoding="utf-8")
        updated = replace_block(original, build_block(rows, args.workbook.name))
        args.main_lua.write_text(updated, encoding="utf-8", newline="\n")
